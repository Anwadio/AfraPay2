      ...
      <div className="grid grid-cols-2 gap-4">
        {stats.map((stat, index) => (
          <div key={index} className="text-center p-3 bg-primary-50 rounded-lg">
            ...
            <p className="text-xs text-primary-700 mb-1">{stat.label}</p>
          </div>
        ))}
      </div>
    </DashboardWidget>
          ...
          <div key={index} className="flex items-center justify-between py-2">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-primary-600">{activity.time}</p>
              </div>
            </div>
...
const WidgetContainer = ({ widgetId, data, config, className, ...props }) => {
  ...
  if (!widget) {
    return (
      <div
        className={cn(
          "p-4 border-2 border-dashed border-primary-200 rounded-lg text-center text-primary-600",
          className
        )}
      >
